-- Owner decision (2026-07-08): the feedback form must use ONLY the Margarita
-- and Sona dashboard results — the live AI chat detections (qa_unanswered_chats
-- / qa_answered_late_chats / qa_overdue_promises) are dropped from ingestion
-- for everyone.
--
-- kk_ingest_problems() now contains just the four manual-review blocks:
--   * sqa_tickets                       → sona:<id>            (source 'sona_review')
--   * sqa_reviews problem w/o ticket    → sona_review:<id>     (source 'sona_review',   0021)
--   * mqa_violations                    → margarita:<id>       (source 'margarita_review')
--   * mqa_evaluations «Критично»/«Плохо» → margarita_eval:<id> (source 'margarita_review', 0021)
-- The live-QA blocks plus their reclassification (0014/0015) and auto-resolve
-- (0012) steps are removed — with no AI rows being refreshed there is nothing
-- left to reclassify or retire. The qa_* RPCs themselves and
-- kk_first_substantive_staff_reply_after() stay in place untouched (the QA
-- dashboards still use them); this only stops COPYING their output into
-- kk_problems.
--
-- One-time cleanup at the end: open, untouched AI rows are retired to the
-- terminal 'auto_resolved' status so they leave the queues and counts. Rows an
-- accountant or reviewer has already acted on (submitted / in_review / fixed /
-- returned / …) are kept — their work stays visible in the review flow — and
-- history (including kk_problem_ratings) is never deleted.
--
-- JS note: the qa mappers in src/lib/ingestion.js (mapUnansweredChat & co) are
-- kept — qaKind() still classifies the HISTORICAL ai rows on the QA-accuracy
-- and Review screens — but they no longer describe live ingestion.
...
-- One-time: retire open, untouched AI detections so they leave the queues and
-- dashboard counts. Rows with accountant/reviewer work in progress are kept.
update public.kk_problems
set status = 'auto_resolved', updated_at = now()
where source = 'ai'
  and status in ('new', 'waiting_for_accountant');
