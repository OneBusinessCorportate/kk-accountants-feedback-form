-- Fix qa_unanswered_chats: promise messages ("I'll check and reply later",
-- "Проверю и отвечу через X минут", etc.) must NOT count as real staff answers.
--
-- Two places needed the fix:
-- 1. last_staff CTE — all 4 branches advance last_staff_at; a promise advances
--    the SLA timer, hiding the client's original question from the unanswered queue.
-- 2. pending_validated NOT EXISTS — this check removes a chat from the queue when
--    any staff message appears after oldest_pending_at; promise messages triggered
--    it even though no real answer was given.
--
-- The existing qa_is_employee_promise() function (already in prod) correctly
-- identifies deferral phrases in Russian / Armenian / English.
...
AS $function$
    ...
    -- A promise ("I'll check and reply later") does NOT count as an answer.
    last_staff AS (
