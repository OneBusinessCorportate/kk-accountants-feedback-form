-- Ingest LIVE QA detections into kk_problems.
--
-- Until now kk_ingest_problems() only pulled manual Margarita/Sona reviews
-- (mqa_violations / sqa_tickets). The bulk of real problems are detected live by
-- the qa_* RPCs that power the dashboards:
--   qa_unanswered_chats     → «Без ответа клиенту»   (no answer)
--   qa_answered_late_chats  → «Поздний ответ клиенту» (late answer)
--   qa_overdue_promises     → «Невыполненное обещание» (broken promise / рассылка не отправлена)
-- This migration adds those three sources so they appear in the feedback form,
-- each attributed to a REAL employee (uuid) where the RPC names one.
--
-- Attribution: a group chat has many staff present (chat_employee_presence lists
-- everyone), so we DON'T use presence. We use the accountant the RPC itself
-- names — accountant_names for unanswered (one problem PER listed accountant, so
-- every responsible accountant sees it), responder_name for late. Overdue
-- promises carry no name, so they stay unassigned (supervisors still see them).
-- Names are resolved to employees by full_name, then alias, then space-insensitive.

create or replace function public.kk_resolve_employee(p_name text)
...
as $$
  ...
  from (
    select e.id, e.full_name, 1 as pri
      from public.employees e
      where public.kk_norm_name(e.full_name) = public.kk_norm_name(p_name)
    union all
    select a.employee_id, a.full_name, 2
      from public.kk_accountant_aliases a
      where a.alias_norm = public.kk_norm_name(p_name)
    union all
    select e.id, e.full_name, 3
      from public.employees e
      where replace(public.kk_norm_name(e.full_name), ' ', '') = replace(public.kk_norm_name(p_name), ' ', '')
  ) s
