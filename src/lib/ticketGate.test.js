...
// Fixed clock — the gate no longer filters by date, so any timestamp counts.
...
describe('selectOutstandingTickets', () => {
  ...
  it('keeps tickets from any day — today, yesterday and older all count', () => {
    const tickets = selectOutstandingTickets({
      problems: [
        ticket({ problem_id: 'today', detected_at: TODAY, problem_title: 'A' }),
        ticket({ problem_id: 'yest', detected_at: YESTERDAY, problem_title: 'B' }),
        ticket({ problem_id: 'old', detected_at: OLDER, problem_title: 'C' }),
      ],
      chats: CHATS,
      access: ACC,
      now: NOW,
...
describe('computeGate', () => {
  // Distinct titles + days so dedup (source+chat+accountant+day+title) keeps them apart.
  const problems = [
    ticket({ problem_id: 'p1', problem_title: 'Проблема 1', detected_at: TODAY }),
    ticket({ problem_id: 'p2', problem_title: 'Проблема 2', detected_at: YESTERDAY }),
    ticket({ problem_id: 'p3', problem_title: 'Проблема 3', detected_at: OLDER }),
  ]
