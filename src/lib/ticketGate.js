// Mandatory "answer ALL of your open tickets first" login gate (pure logic).
//
// Owner rule: a regular accountant must NOT see any normal page (Дашборд,
// Задачи, Клиенты, Отчётность, …) until EVERY ticket assigned to them has been
// answered — each either accepted («Ознакомлен») or appealed («Подать
// апелляцию»). There is NO date window: covering only yesterday's tickets is
// not enough, the accountant must clear their entire outstanding queue before
// the platform unlocks.
//
...
// Every RELEVANT ticket assigned to the accountant, regardless of date.
//
...
// Split outstanding tickets into answered / unanswered and produce progress.
