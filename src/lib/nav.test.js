...
describe('visibleNavLinks', () => {
  it('shows only Dashboard + Report + Accountant + Tasks + Clients + Mailings + Accounting to a non-manager', () => {
    const tos = visibleNavLinks(false).map((l) => l.to)
    expect(tos).toEqual(['/', '/report', '/accountant', '/tasks', '/clients', '/mailings', '/accounting'])
  })

  it('shows every link to a manager', () => {
    const tos = visibleNavLinks(true).map((l) => l.to)
    expect(tos).toEqual(['/', '/report', '/accountant', '/tasks', '/clients', '/mailings', '/accounting', '/management', '/reports'])
  })
