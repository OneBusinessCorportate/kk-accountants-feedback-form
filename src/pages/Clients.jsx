...
export default function Clients() {
  ...
  const [creating, setCreating] = useState(null) // "{clientName}:{type}" while saving
  ...
  // Clicking a "not created" cell instantly creates that task for the client
  ...
  function taskCell(client, type) {
    ...
    // Not created — clickable to instantly create
    return (
      <button
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">
        Проблемные чаты и статус задач по каждому клиенту. Нажмите «+» в ячейке, чтобы создать задачу.
      </p>
                ...
                return (
                  <React.Fragment key={c.name}>
                    <tr
                      ...
                      style={{
                        cursor: 'pointer',
                        background: noMailing && openProblems.length > 0 ? '#fffbf0' : undefined,
                      }}
                    >
                      ...
                      <td>
                        ...
                        {uniqueChats.length > 3 && (
                          <span style={{ color: 'var(--muted)', fontSize: 12 }}>+{uniqueChats.length - 3}</span>
                        )}
                      ...
                      <tr>
                        <td colSpan={3 + CHECK_TYPES.length} style={{ background: 'var(--bg)', padding: '10px 16px' }}>
                          {c.problems.map((p) => (
                            <div
                              key={p.problem_id}
                              style={{ display: 'flex', gap: 10, alignItems: 'baseline', marginBottom: 6, fontSize: 13 }}
                            >
                              ...
                              {p.chat_link && (
                                <a href={p.chat_link} target="_blank" rel="noopener noreferrer">→ чат</a>
                              )}
                            </div>
                          ))}
                        </td>
